...
def compute_stats(pred_transforms, data_loader):
    ...
    pred_transforms = pred_transforms.cpu()
    num_processed = 0
...
def summarize_metrics(metrics):
    ...
    for k in metrics:
        ...
        if k.startswith('ad'):
            ...
            if k.startswith('r'):
                summarized[k + '_5deg'] = np.mean(metrics[k] < 5.0)
                summarized[k + '_10deg'] = np.mean(metrics[k] < 10.0)
            elif k.startswith('t'):
                summarized[k + '_5cm'] = np.mean(metrics[k] < 0.05)
                summarized[k + '_10cm'] = np.mean(metrics[k] < 0.1)
